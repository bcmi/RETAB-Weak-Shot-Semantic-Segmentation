...
def do_python_eval(predict_folder, targetpixel_folder, gt_folder, name_list, num_cls=21, input_type='png', threshold=1.0, printlog=False):
    TP = []
    P = []
    T = []
...
if __name__ == '__main__':
    ...
    if not args.curve:
        loglist = do_python_eval(args.predict_dir, args.targetpixel_dir, args.gt_dir, name_list, 21, args.type, args.t, printlog=True)
        writelog(args.logfile, loglist, args.comment)
    else:
        resall = []
        novelall = []
        loglistall = []
        start_id, end_id = 5, 25
        for i in range(start_id, end_id+1):
            t = i/100.0
            loglist = do_python_eval(args.predict_dir, args.targetpixel_dir, args.gt_dir, name_list, 21, args.type, t)
            resall.append(loglist['mIoU'])
            novel_mIoU = loglist['novel-mIoU']
            novelall.append(novel_mIoU)
            print('%d/60 background score: %.3f\tmIoU: %.3f%% novel-mIoU: %.3f%%'%(i, t, loglist['mIoU'], novel_mIoU))
            # loglist['novel-mIoU'] = novel_mIoU
            loglist['%d/60_background_score'%(i)] = '%.3f'%(t)
