...
if __name__ == '__main__':
    ...
    parser.add_argument("--network", default="network.resnet38_aff_mask_new", type=str)
    ...
    parser.add_argument("--out_rw_cam", default='out_rw_cam', type=str)
    ...
    assert (args.targetpixel_dir!=None and os.path.exists(args.targetpixel_dir))
    ...
    for iter, (name, img) in enumerate(infer_data_loader):
        ...
        ## start add ##
        if args.targetpixel_dir != 'None':
            targetpixel_file = os.path.join(args.targetpixel_dir,'%s.png'%name)
            targetpixel = np.array(Image.open(targetpixel_file)).astype(np.int32)
            nonboundary = torch.LongTensor(np.where(targetpixel == 0, 0, 1)).unsqueeze(0).unsqueeze(0)
        ##  end  add ##

        with torch.no_grad():
            aff_mat = torch.pow(model.forward(img.cuda(), nonboundary, True, 'second'), args.beta)
...
#                cam_full_arr = torch.from_numpy(cam_full_arr).view(1, 21, dheight, dwidth).cuda()
            ...
            cam_dict = {}
            # cam_dict[0] = cam_rw[0, 0, :, :].cpu().numpy()[:orig_shape[2], :orig_shape[3]]
            # for k in cam.keys():
            #     cam_dict[k+1] = cam_rw[0, k+1, :, :].cpu().numpy()[:orig_shape[2], :orig_shape[3]]
            for k in cam.keys():
                cam_dict[k] = cam_rw[0, k, :, :].cpu().numpy()[:orig_shape[2], :orig_shape[3]]
            np.save(os.path.join(args.out_rw_cam, name + '.npy'), cam_dict)
