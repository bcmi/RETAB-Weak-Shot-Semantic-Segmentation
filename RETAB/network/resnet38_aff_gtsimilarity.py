...
class Net(network.resnet38d.Net):
    ...
    def forward(self, x, label, to_dense=False):

        assert (x.shape[0] == label.shape[0] and x.shape[2] == label.shape[1] and x.shape[3] == label.shape[2])
        ...
        label = F.interpolate(label.unsqueeze(1).float(), size=(x.shape[2], x.shape[3]), mode='nearest').long()
        label = label.view(label.size(0), label.size(1), -1).contiguous()
        ...
        ff = torch.index_select(label, dim=2, index=ind_from.cuda(non_blocking=True)).long()
        ft = torch.index_select(label, dim=2, index=ind_to.cuda(non_blocking=True)).long()

        ff = torch.unsqueeze(ff, dim=2)
        ft = ft.view(ft.size(0), ft.size(1), -1, ff.size(3))
        ff = ff.repeat(1, 1, ft.shape[2], 1)
        assert (ff.shape == ft.shape)

        ones = torch.ones(ft.shape).cuda()
        zeros = torch.zeros(ft.shape).cuda()

        ww = torch.where(ff == ft, ones, zeros).float()
        aff = ww.squeeze(1)
