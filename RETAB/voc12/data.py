...
class VOC12ImageGtDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        # label = scipy.misc.imread(label_path).astype(np.int64)
        label = np.asarray(PIL.Image.open(label_path), dtype=np.int32)

        return name, img, label


class VOC12ImageBoundaryDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        label = np.where(label[:,:,0].astype(np.int32)==255, 1, 0).astype(np.int32)

        return name, img, label


class VOC12ImageBoundaryDatasetNew(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        return name, img, label[:,:,0].astype(np.int64)
...
class VOC12AffDatasetMask(VOC12ImageDataset):

    def __init__(self, img_name_list_path, label_la_dir, label_ha_dir, mask_dir, cropsize, voc12_root, radius=5,
                 joint_transform_list=None, img_transform_list=None, label_transform_list=None):
        super().__init__(img_name_list_path, voc12_root, transform=None)
    ...
    def __getitem__(self, idx):
        ...
        ## start add ##
        targetpixel_file = os.path.join(self.mask_dir, '%s.png'%name)
        targetpixel = np.array(PIL.Image.open(targetpixel_file)).astype(np.int32)
        mask = np.where(targetpixel == 0, 0, 1).astype(np.int32)
        ...
        for joint_transform, img_transform, label_transform \
            ...
            if joint_transform:
                img_label_mask = np.concatenate((img, label, mask), axis=-1)
                img_label_mask = joint_transform(img_label_mask)
                img = img_label_mask[..., :3]
                label = img_label_mask[..., 3:-1]
                mask = img_label_mask[..., -1:]
        ...
        ## start add ##
        label = imutils.AvgPool2d(8)(label)
        mask = torch.LongTensor(mask[:,:,0].astype(np.int32)).unsqueeze(0).unsqueeze(0)
        mask = F.interpolate(mask.float(), size=label.shape[:2], mode="nearest")
        mask = mask.long().squeeze(0).squeeze(0).contiguous().numpy().astype(np.int32)
        ##  end  add ##
...
class VOC12AffGtDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        # new
        label = torch.FloatTensor(label[:,:,0]).unsqueeze(0).unsqueeze(0)
        label = F.interpolate(label, size=(label.shape[2]//8, label.shape[3]//8), mode="nearest").long()
        label = label[0,0,:,:].numpy().astype(np.uint8)

        label = self.extract_aff_lab_func(label)

        return img, label

class VOC12AffMixedDataset(VOC12ImageDataset):

    def __init__(self, img_name_list_path, label_la_dir, label_ha_dir, label_dir, base_dir, novel_dir, cropsize, voc12_root, radius=5,
                 joint_transform_list=None, img_transform_list=None, label_transform_list=None):
        super().__init__(img_name_list_path, voc12_root, transform=None)
        ...
        self.base_sample_ids = np.load(base_dir).tolist()
        self.novel_sample_ids = np.load(novel_dir).tolist()
    ...
    def __getitem__(self, idx):
        ...
        if name in self.base_sample_ids:
            is_Novel = False
        else:
            assert (name in self.novel_sample_ids)
            is_Novel = True
...
class VOC12AffMixedDatasetMask(VOC12ImageDataset):

    def __init__(self, img_name_list_path, label_la_dir, label_ha_dir, mask_dir, label_dir, base_dir, novel_dir, cropsize, voc12_root, radius=5,
                 joint_transform_list=None, img_transform_list=None, label_transform_list=None):
        super().__init__(img_name_list_path, voc12_root, transform=None)
